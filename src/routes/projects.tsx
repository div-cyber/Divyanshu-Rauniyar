...
const projects: Array<{
...
}> = [
  {
    ...
    status: "Active",
    description:
      "An engineering studio building thoughtful software and AI products. Serves as the umbrella for client work, internal tooling, and a growing library of open-source bits.",
  ...
  },
  {
    ...
    status: "Shipped",
    description:
      "An exam preparation platform built for Nepali students — adaptive practice, analytics, and well-designed lessons that respect the learner's time.",
  ...
  },
  {
    ...
    status: "Active",
    description:
      "Tools for athletes and coaches — track training, monitor performance, and surface insights with a calm, data-first interface.",
  ...
  },
  {
    ...
    status: "Active",
    description:
      "A community-powered alerting system for missing persons across Nepal. Real-time notifications, verified reports, and a respectful UX for a hard problem.",
  ...
  },
  {
    ...
    status: "In Progress",
    description:
      "A single workbench to compare, route, and evaluate responses across leading AI providers. Built for developers who actually run evals.",
  ...
  },
  {
    ...
    status: "Shipped",
    description:
      "An AI-assisted coding companion focused on the boring 80% — scaffolds, refactors, and review nudges that respect your codebase.",
  ...
  },
  {
    ...
    status: "Archived",
    description:
      "A collection of home-grown IoT experiments — sensors, dashboards, and small automations that taught me a lot about embedded systems.",
  ...
  },
  {
    ...
    status: "Active",
    description:
      "This very site — a calm, content-first personal headquarters for articles, notes, and selected work.",
...
export default function ProjectsPage() {
  ...
  const intro =
    section?.body ??
    "A selection of products, tools, and experiments — from BLACKBYTE work to weekend hacks. Each one taught me something I still use today.";
  ...
  const featured = githubProjects.length
    ? githubProjects.slice(0, 4)
    : projects.filter((p) => p.featured);
  const rest = githubProjects.length
    ? githubProjects.slice(4)
    : projects.filter((p) => !p.featured);
    ...
    <div className="mx-auto max-w-3xl px-5 py-8">
      ...
      <section className="mt-16">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          {(githubLoading || githubError ? projects.filter((p) => p.featured) : featured).map(
            (p) => (
          ...
          <div>
            ...
            {githubLoading ? (
              ...
              <p className="text-xs text-destructive">
                Failed to load GitHub projects. Showing local fallback.
              </p>
            ) : null}
          </div>
          <a
            href="https://github.com/div-cyber?tab=repositories"
            target="_blank"
            rel="noreferrer"
      ...
      <section className="mt-16 rounded-2xl border border-border bg-secondary/50 px-6 py-10 text-center">
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-muted-foreground">
          BLACKBYTE takes on a small number of partnerships each year. If you have an idea worth
...
function StatusBadge({ status }: { status: Status }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[0.65rem] font-semibold uppercase tracking-wider ${map[status]}`}
    >
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap gap-1.5">
        {project.stack.map((s) => (
          <span
            key={s}
            className="rounded-md border border-border bg-background px-2 py-0.5 text-[0.7rem] font-medium text-muted-foreground"
          >
            {s}
          </span>
        ))}
